...
export default function ActivityScreen({ projectId }) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16 }}>
      <Card>
        <div style={{ padding: 16, borderBottom: `1px solid ${C.line}`, display: 'flex', gap: 8 }}>
          <TabBtn active>All changes</TabBtn>
          <TabBtn>Schedule</TabBtn>
          <TabBtn>Creative</TabBtn>
          <TabBtn>People</TabBtn>
          <TabBtn>AI</TabBtn>
        </div>
        <div>
          {displayItems.map((it, i) => {
            ...
            return (
              <div key={it.id} style={{ display: 'grid', gridTemplateColumns: '60px 1fr', gap: 16, padding: '14px 20px', borderBottom: i < displayItems.length - 1 ? `1px solid ${C.line}` : 'none' }}>
                <div style={{ fontSize: 11, color: C.muted, fontFamily: '"IBM Plex Mono", monospace' }}>
                  {it.created_at ? formatTime(it.created_at) : ['09:42','09:28','09:15','08:50','08:34','08:12','07:55','07:30'][i] || ''}
                </div>
                <div>
      ...
      <Card>
        ...
        <div style={{ padding: 16 }}>
          <ImpactRow label="Time saved today" val="+42 min" tone="ok"/>
          <ImpactRow label="Changes accepted" val="7 of 9"/>
